...
export default async (req: Request, context: Context) => {
  ...
  const SUPA_URL = Netlify.env.get("aSUPABASE_URL") || Netlify.env.get("SUPABASE_URL") || "";
  const SUPA_KEY = Netlify.env.get("SUPABASE_ANON_KEY") || Netlify.env.get("SUPABASE_KEY") || "";
  ...
  if (req.method === "POST" && path.startsWith("/submit/")) {
    ...
    if (!entry?.id) return cors({ error: "Etablissement non trouve: " + slug }, 404, req);
    ...
    if (!res.ok) {
      ...
      return cors({ error: "DB error", detail: err.substring(0, 200) }, 500, req);
    }
    const data = await res.json();
    return cors({ ok: true, tracking_code: code, report_id: data[0]?.id }, 201, req);
  }

  // GET /api/reports/list/:slug
  if (req.method === "GET" && path.startsWith("/list/")) {
    ...
    const res = await fetch(SUPA_URL + "/rest/v1/reports?school_id=eq." + entry.id + "&order=created_at.desc&limit=200", {
  ...
  if (req.method === "GET" && path.startsWith("/track/")) {
    ...
    const res = await fetch(SUPA_URL + "/rest/v1/reports?tracking_code=eq." + code + "&select=tracking_code,status,admin_reply,created_at,type&limit=1", {
      headers: { "apikey": SUPA_KEY, "Authorization": "Bearer " + SUPA_KEY },
    });
    const data = await res.json();

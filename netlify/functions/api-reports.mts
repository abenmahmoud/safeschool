...
export default async (req: Request, context: Context) => {
  ...
  if (req.method === "POST" && path.startsWith("/submit/")) {
    ...
    const report = {
      ...
      reporter_email: String(body.reporter_email || "").substring(0, 200),
      classe: String(body.classe || body.class_name || "").substring(0, 100),
      status: "nouveau",
      source_channel: "web",
    ...
    if (!res.ok) {
      ...
      return cors({ error: "Erreur base de donnees: " + err }, 500, req);
    }
  ...
  if (req.method === "GET" && path.startsWith("/list/")) {
    ...
    const res = await fetch(SUPABASE_URL + "/rest/v1/reports?school_id=eq." + entry.id + "&order=created_at.desc&limit=200", {
  ...
  if (req.method === "GET" && path.startsWith("/track/")) {
    ...
    const res = await fetch(SUPABASE_URL + "/rest/v1/reports?tracking_code=eq." + code + "&select=tracking_code,status,admin_reply,created_at,type", {
      headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY },
    });
    const data = await res.json();

  ...
  // === FICHIERS D'UN SIGNALEMENT (admin) ===
  if (req.method === 'GET' && path.startsWith('/report-files/')) {
    const slugF = (path.split('/report-files/')[1] || '').split('?')[0].toLowerCase();
    const urlF = new URL(req.url);
    const reportId = urlF.searchParams.get('report_id');
    if (!slugF || !reportId) return cors({ error: 'Params requis' }, 400, req);
    const suF = Netlify.env.get('aSUPABASE_URL') || Netlify.env.get('SUPABASE_URL') || '';
    const skF = Netlify.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
    const filesRes = await fetch(suF + '/rest/v1/report_files?report_id=eq.' + reportId + '&select=*', { headers: { 'apikey': skF, 'Authorization': 'Bearer ' + skF } });
    const files: any[] = await filesRes.json();
    const signed = await Promise.all(files.map(async (f: any) => {
      try {
        const sr = await fetch(suF + '/storage/v1/object/sign/report-files/' + f.file_path, { method: 'POST', headers: { 'apikey': skF, 'Authorization': 'Bearer ' + skF, 'Content-Type': 'application/json' }, body: JSON.stringify({ expiresIn: 3600 }) });
        const sd = await sr.json();
        return { ...f, signed_url: sr.ok ? suF + '/storage/v1' + sd.signedURL : null };
      } catch { return { ...f, signed_url: null }; }
    }));
    return cors({ ok: true, files: signed }, 200, req);
  }

  // === MISE À JOUR D'UN SIGNALEMENT (admin) ===
  if (req.method === 'POST' && path.startsWith('/update-report/')) {
    const slugU = (path.split('/update-report/')[1] || '').split('?')[0].toLowerCase();
    let bodyU: any = {};
    try { bodyU = await req.json(); } catch { return cors({ error: 'Invalide' }, 400, req); }
    const { report_id, status, admin_reply, admin_note } = bodyU;
    if (!report_id) return cors({ error: 'report_id requis' }, 400, req);
    const suU = Netlify.env.get('aSUPABASE_URL') || Netlify.env.get('SUPABASE_URL') || '';
    const skU = Netlify.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
    const upd: any = { updated_at: new Date().toISOString() };
    if (status) upd.status = status;
    if (admin_reply !== undefined) upd.admin_reply = admin_reply;
    if (admin_note !== undefined) upd.admin_note = admin_note;
    const ru = await fetch(suU + '/rest/v1/reports?id=eq.' + report_id, { method: 'PATCH', headers: { 'apikey': skU, 'Authorization': 'Bearer ' + skU, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' }, body: JSON.stringify(upd) });
    if (!ru.ok) return cors({ error: 'Erreur mise a jour' }, 500, req);

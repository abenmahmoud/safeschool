...
export default async (req: Request, context: Context) => {
  // === ADMIN JWT LOGIN - PRIORITAIRE ===
  const urlObj = new URL(req.url);
  const pathObj = urlObj.pathname.replace('/api/establishments', '');
  if (req.method === 'POST' && pathObj.startsWith('/admin-jwt/')) {
    const slugJ = (pathObj.split('/admin-jwt/')[1] || '').split('?')[0].toLowerCase();
    let bodyJ: any = {};
    try { bodyJ = await req.json(); } catch {}
    const codeJ = String(bodyJ.admin_code || '').trim();
    if (!slugJ || !codeJ) return cors({ error: 'Params requis' }, 400, req);
    const suJ = Netlify.env.get('aSUPABASE_URL') || Netlify.env.get('SUPABASE_URL') || '';
    const skJ = Netlify.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
    const resJ = await fetch(suJ + '/rest/v1/schools?slug=eq.' + slugJ + '&select=id,name,slug,is_active,admin_code,plan_code', { headers: { apikey: skJ, Authorization: 'Bearer ' + skJ } });
    ...
    const pJ = btoa(JSON.stringify({ slug: slugJ, school_id: scJ.id, school_name: scJ.name, role: 'admin', iat: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + 86400 })).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
    const mJ = hJ + '.' + pJ;
    ...
    return new Response(JSON.stringify({ ok: true, token: tokJ, school_name: scJ.name, role: 'admin' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Set-Cookie': 'ss_admin_token=' + tokJ + '; HttpOnly; SameSite=Strict; Path=/; Max-Age=86400' }
    });
  }
...
export default async (req: Request, context: Context) => {
  ...
  if (req.method === 'POST' && path.startsWith('/admin-jwt/')) {
    ...
    const sJ = await crypto.subtle.sign('HMAC', kJ, new TextEncoder().encode(mJ));
    const tokJ = mJ + '.' + btoa(String.fromCharCode(...new Uint8Array(sJ))).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
    return new Response(JSON.stringify({ ok: true, token: tokJ, school_name: scJ.name, role: 'admin' }), { status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Set-Cookie': 'ss_admin_token=' + tokJ + '; HttpOnly; SameSite=Strict; Path=/; Max-Age=86400' } });
  }

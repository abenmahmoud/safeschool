...
// =============================================
// SOUS-ADMIN : bouton + modal + sauvegarde
// =============================================
...
window.openSubAdminModal = function() {
  ...
  var html = '<div style="background:#fff;border-radius:20px;padding:28px 24px;width:90%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,.4);">';
  html += '<h2 style="margin:0 0 4px;font-size:18px;font-weight:700;color:#0f1428;">Ajouter un sous-admin</h2>';
  html += '<p style="margin:0 0 18px;color:#6b7280;font-size:13px;">Accès restreint — voit uniquement ses incidents</p>';
  html += '<div id="ssa-err" style="display:none;background:#fef2f2;color:#dc2626;border-radius:8px;padding:10px;margin-bottom:14px;font-size:13px;"></div>';
  html += '<label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px;">Prénom et Nom *</label>';
  html += '<input id="ssa-name" type="text" placeholder="Mme Martin Sophie" style="width:100%;box-sizing:border-box;padding:11px 13px;border:1.5px solid #e5e7eb;border-radius:10px;font-size:14px;margin-bottom:13px;outline:none;">';
  html += '<label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px;">Rôle *</label>';
  html += '<select id="ssa-role" style="width:100%;box-sizing:border-box;padding:11px 13px;border:1.5px solid #e5e7eb;border-radius:10px;font-size:14px;margin-bottom:13px;background:#fff;outline:none;">';
  html += '<option value="CPE">CPE — Conseiller Principal d’Éducation</option>';
  html += '<option value="PSY">Psychologue scolaire</option>';
  html += '<option value="PROF">Professeur référent pHARe</option>';
  html += '<option value="INF">Infirmier(e) scolaire</option>';
  html += '<option value="DIR">Direction</option>';
  html += '</select>';
  html += '<label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px;">Email</label>';
  html += '<input id="ssa-email" type="email" placeholder="prenom.nom@lycee.fr" style="width:100%;box-sizing:border-box;padding:11px 13px;border:1.5px solid #e5e7eb;border-radius:10px;font-size:14px;margin-bottom:20px;outline:none;">';
  html += '<div style="display:flex;gap:10px;">';
  html += '<button onclick="document.getElementById('ss-subadmin-overlay').remove()" style="flex:1;padding:12px;background:#f9fafb;color:#6b7280;border:1px solid #e5e7eb;border-radius:10px;font-size:14px;cursor:pointer;">Annuler</button>';
  html += '<button id="ssa-btn" onclick="window.saveSubAdmin()" style="flex:2;padding:12px;background:#7c3aed;color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;">Créer le compte</button>';
  html += '</div></div>';
  ov.innerHTML = html;
...
window.saveSubAdmin = async function() {
  ...
  try {
    var res = await fetch('/api/establishments/add-subadmin/' + slug, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-sa-token': '', 'x-admin-code': adminData.admin_code || adminData.admin_password || '' },
...
Communiquez ce code à ' + name + ' pour qu’il puisse se connecter.');

...
function detectTenant(){var h=location.hostname,m=h.match(/^([a-z0-9-]+)\.safeschool\.fr$/);return m?m[1]:null;}
window.SS_TENANT=detectTenant();

window.DB={
  _sb:function(){return!!(SUPABASE_URL&&SUPABASE_KEY);},
  getReports:async function(sid){
    if(this._sb()){try{var r=await fetch(SUPABASE_URL+'/rest/v1/reports?school_id=eq.'+sid+'&order=created_at.desc',{headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+SUPABASE_KEY}});return r.ok?await r.json():[];}catch(e){return[];}}
  ...
  saveReport:async function(rpt){
    ...
    if(this._sb()){try{await fetch(SUPABASE_URL+'/rest/v1/reports',{method:'POST',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+SUPABASE_KEY,'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify(Object.assign({},rpt,{school_id:sid}))});return;}catch(e){}}
  ...
  updateStatus:async function(id,status,sid){
    if(this._sb()){try{await fetch(SUPABASE_URL+'/rest/v1/reports?id=eq.'+id,{method:'PATCH',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+SUPABASE_KEY,'Content-Type':'application/json'},body:JSON.stringify({status:status})});return;}catch(e){}}
    var key='ss_rpt_'+sid,list=JSON.parse(localStorage.getItem(key)||'[]');
  ...
  getStats:async function(sid){
    var all=await this.getReports(sid),now=new Date();
    ...
    all.forEach(function(r){var t=r.type||'autre';byType[t]=(byType[t]||0)+1;var s=r.status||'nouveau';if(byStatus[s]!==undefined)byStatus[s]++;else byStatus[s]=1;});
    var trend=[];
    for(var i=0;i<7;i++){var d=new Date(now);d.setDate(d.getDate()-(6-i));var ds=d.toISOString().slice(0,10);var dn=d.toLocaleDateString('fr-FR',{weekday:'short'}).slice(0,3);var cnt=all.filter(function(r){return r.created_at&&r.created_at.startsWith(ds);}).length;trend.push({date:ds,day:dn,count:cnt});}
    return{total:all.length,thisMonth:thisM.length,byType:byType,byStatus:byStatus,trend:trend,recent:all.slice(0,20)};
  }
};

(function seedDemo(){
  var sid=localStorage.getItem('ss_current_etab')||'demo',key='ss_rpt_'+sid;
  if(localStorage.getItem(key))return;
  var types=['verbal','physique','cyber','exclusion','autre'];
  var classes=['6eme A','6eme B','5eme A','4eme A','4eme B','3eme A','3eme B'];
  var descs=['Moqueries repetees en classe','Messages blessants sur les reseaux','Mise a l ecart systematique','Bousculades dans les couloirs','Screenshots partages sans consentement','Insultes pendant la recreation','Commentaires degradants sur Instagram','Exclusion des conversations de groupe'];
  ...
  for(var i=0;i<16;i++){var d=new Date(now);d.setDate(d.getDate()-Math.floor(Math.random()*30));list.push({id:'demo_'+i,type:types[i%5],classe:classes[i%7],urgence:urgences[i%5],status:statuses[i%6],anonymous:true,description:descs[i%8],school_id:sid,created_at:d.toISOString()});}
  localStorage.setItem(key,JSON.stringify(list));
})();

var TYPE_LBL={verbal:'Verbal',physique:'Physique',cyber:'Cyber',exclusion:'Exclusion',autre:'Autre'};
var TYPE_EMJ={verbal:'chat',physique:'fist',cyber:'laptop',exclusion:'no',autre:'memo'};
var TYPE_COL={verbal:'#dc2626',physique:'#ea580c',cyber:'#7c3aed',exclusion:'#0891b2',autre:'#64748b'};
var STATUS_LBL={nouveau:'Nouveau','en-cours':'En cours',traite:'Traite',archive:'Archive'};
...
window.renderRptList=function(list){
  ...
  return list.map(function(r){
    ...
    return '<div class="rpt-item" onclick="openRpt(''+r.id+'')">'+
      '<div class="rpt-row1"><span class="tag tag-'+(r.type||'autre')+'">'+(TYPE_LBL[r.type||'autre']||r.type)+'</span>'+
      '<span class="tag tag-'+(r.status||'nouveau')+'">'+(STATUS_LBL[r.status||'nouveau']||r.status)+'</span></div>'+
...
window.fltRpt=async function(status,btn){
  document.querySelectorAll('.flt-btn').forEach(function(b){b.classList.remove('on');});btn.classList.add('on');
  ...
  var el=document.getElementById('rpt-list');if(el)el.innerHTML=window.renderRptList(f.slice(0,20));
};

window.openRpt=function(id){
  var all=window._allRptsAll||[];var r=null;
  for(var i=0;i<all.length;i++){if(all[i].id===id){r=all[i];break;}}if(!r)return;
  var sid=localStorage.getItem('ss_current_etab')||'demo';
  ...
  m.innerHTML='<div class="rpt-modal">'+
    ...
    '<div style="font-size:1.05rem;font-weight:800;color:#1e293b">'+(TYPE_LBL[r.type]||r.type||'Signalement')+'</div>'+
    ...
    '<div style="font-size:.75rem;font-weight:700;color:#94a3b8;margin-bottom:10px">CHANGER LE STATUT</div>'+
    '<div class="st-btns">'+
...
window.chgSt=async function(id,status,sid){
  ...
  var el=document.querySelector('[data-dash-content]');if(el)window.renderDashboardV3(el);
};
...
window.renderDashboardV3=async function(el){
  ...
  var maxBar=1;S.trend.forEach(function(d){if(d.count>maxBar)maxBar=d.count;});
  var bars='';S.trend.forEach(function(d,i){var h=Math.max(3,Math.round(d.count/maxBar*44)),today=i===6;bars+='<div class="mini-bar-col"><div class="mini-cnt'+(today?' today':'')+'">'+( d.count||'')+'</div><div class="mini-bar'+(today?' today':'')+'" style="height:'+h+'px"></div><div class="mini-day" style="color:'+(today?'#4f46e5':'#94a3b8')+'">'+d.day+'</div></div>';});
  var types=Object.entries(S.byType).sort(function(a,b){return b[1]-a[1];}).slice(0,5);
  var tot=S.total||1,r=32,circ=2*Math.PI*r,offset=0,segs='',leg='';
  types.forEach(function(e){var t=e[0],n=e[1],da=n/tot*circ;segs+='<circle cx="42" cy="42" r="'+r+'" fill="none" stroke="'+(TYPE_COL[t]||'#94a3b8')+'" stroke-width="9" stroke-dasharray="'+da.toFixed(2)+' '+(circ-da).toFixed(2)+'" stroke-dashoffset="'+(-offset).toFixed(2)+'"/>';offset+=da;});
  types.slice(0,4).forEach(function(e){var t=e[0],n=e[1];leg+='<div class="leg-row"><div class="leg-dot" style="background:'+(TYPE_COL[t]||'#94a3b8')+'"></div><div class="leg-lbl">'+(TYPE_LBL[t]||t)+'</div><div class="leg-val">'+n+'</div></div>';});
  ...
  var empty=S.total===0?'<div style="text-align:center;padding:40px 20px"><div style="font-size:2.5rem">ok</div><div style="font-weight:700;color:#64748b;margin-top:8px">Aucun signalement</div></div>':'';
  ...
  el.innerHTML='<div style="background:#f8fafc;min-height:100%;padding-bottom:32px">'+
    ...
    '<div class="kpi-card" style="--kpi-color:#4f46e5"><div class="kpi-icon">total</div><div class="kpi-label">Total</div><div class="kpi-num">'+S.total+'</div><div class="kpi-trend neu">signalements</div></div>'+
...
console.log('[SafeSchool v3] dashboard-v3.js OK');